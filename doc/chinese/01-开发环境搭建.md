...
```
docker exec -it mysql mysql -uroot -pPXDN93VRKUm8TeE7 -e "CREATE DATABASE IF NOT EXISTS looklook_order; CREATE DATABASE IF NOT EXISTS looklook_payment; CREATE DATABASE IF NOT EXISTS looklook_travel; CREATE DATABASE IF NOT EXISTS looklook_usercenter;"
docker exec -i mysql mysql -uroot -pPXDN93VRKUm8TeE7 looklook_order < deploy\sql\looklook_order.sql

docker exec -i mysql mysql -uroot -pPXDN93VRKUm8TeE7 looklook_payment < deploy\sql\looklook_payment.sql

docker exec -i mysql mysql -uroot -pPXDN93VRKUm8TeE7 looklook_travel < deploy\sql\looklook_travel.sql

docker exec -i mysql mysql -uroot -pPXDN93VRKUm8TeE7 looklook_usercenter < deploy\sql\looklook_usercenter.sql
```

```
插入压测数据
docker exec -it mysql mysql -uroot -pPXDN93VRKUm8TeE7 -e "INSERT INTO `homestay` (
    `id`, `title`, `sub_title`, `banner`, `info`, `people_num`, 
    `homestay_business_id`, `user_id`, `row_state`, `row_type`, 
    `food_info`, `food_price`, `homestay_price`, `market_homestay_price`
) VALUES (
    1001, '压测专属海景房', 'QPS破万不是梦', 'https://example.com/banner.jpg', '这是一条专门用于高并发防击穿测试的民宿数据', 2, 
    1, 1, 1, 0, 
    '包含双人豪华早餐', 5000, 29900, 39900
);"
```
